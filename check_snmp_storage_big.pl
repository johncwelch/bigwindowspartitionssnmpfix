...
for ($i=0;$i<$num_int;$i++) {
  ...
  ###start of john's code to deal with size value wraps in total storage available
  
  my $temptotalsize = $$result{$size_table . $tindex[$i]}; #value of what's in the size table for the index we're looking at
  
  my $to = $$result{$size_table . $tindex[$i]} * ( ( 100 - $o_reserve ) / 100 ) * $$result{$alloc_units . $tindex[$i]} / $output_metric_val; #calculated value
 
   ##declare shit, life is too short to dick with scope
  my $newusedsizevalue = 0;
  my $newtotalsizevalue = 0;
 
  if ($temptotalsize < 0) {#checking for negative numbers here
  	##initial conversion to a better int value
  	$newtotalsizevalue = ((2147483647 * 2)-(-($temptotalsize)));
 	#if we don't have a negative number in the size, then the already calculated $to is fine, and we don't need to change it. if it's negative, then we need
	#to change it.
  	$to = $newtotalsizevalue * ( ( 100 - $o_reserve ) / 100 ) * $$result{$alloc_units . $tindex[$i]} / $output_metric_val;
  } else {
	$newtotalsizevalue = $to;
  }
  
 
   my $bu = $$result{$used_table . $tindex[$i]} *  $$result{$alloc_units . $tindex[$i]} / $output_metric_val;
  # print "used bytes is: $bu\n\n";
  
  ##check for negative numbers in space used, since as things get bigger, this is GOING to happen.
  
  my $tempusedsize = $$result{$used_table . $tindex[$i]};
 
  if ($tempusedsize < 0) { #we ONLY need to take action here if used size is negative. Unlikely to happen soon, but a real possibility
  	$newusedsizevalue = ((2147483647 * 2)-(-($tempusedsize)));
  	$bu = $newusedsizevalue *  $$result{$alloc_units . $tindex[$i]} / $output_metric_val;
  } else {
  	$newusedsizevalue = $bu; #handles positive cases for pu calculations
  }
 ...
 # print "$bu , $to \n\n";
  ...
  ##original plugin PU calculations
 # if ( $$result{$used_table . $tindex[$i]} != 0 ) {
...
#  } 
 ...
 if ($o_type eq "pu") { # Checks % used
  ...
  if ($o_type eq 'bu') { # Checks MBytes used
	...
	if (!defined($o_shortL[0]) || ($locstate==1)) { # print full output if warn or critical state
      $output.=sprintf("%s: %.0f%sB used/%.0f%sB (%.0f%%) ",$descr[$i],$bu,$output_metric,$to,$output_metric,$pu);
    } elsif ($o_shortL[0] == 1) {
  ...
  if ($o_type eq 'bl') {
	...
	if (!defined($o_shortL[0]) || ($locstate==1)) { # print full output if warn or critical state
      $output.=sprintf ("%s: %.0f%sB left/%.0f%sB (%.0f%%) ",$descr[$i],$bl,$output_metric,$to,$output_metric,$pl);
    } elsif ($o_shortL[0] == 1) {
